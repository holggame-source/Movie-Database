...
export default function EditFilmScreen() {
  ...
  useEffect(() => {
    if (film) {
      setTitle(film.title);
      setAlternateTitles(film.alternateTitles || []);
      setFilmType(film.type || 'movie');
      setYear(film.year?.toString() || '');
      setStudio(film.studio || '');
      setPosterUri(film.posterUri);
      setRating(film.rating || 0);
      setSelectedCategories(film.categoryIds);
      setSelectedActresses(film.actressIds);
      setNotes(film.notes || '');
    }
  }, [film]);
  ...
  const toggleActress = (actId: string) => {
